...
class Patient(object):
    ...
    def update(self):
        ...
        copy = surviving_bacteria.copy()

        for i in range(len(copy)):
            try:
                new_bacteria = copy[i].reproduce(pop_density)
...
def simulation_without_antibiotic(num_bacteria,
    ...
    for i in range(num_trials):
        simple_bacteria = [SimpleBacteria(birth_prob, death_prob)] * num_bacteria
        patient = Patient(simple_bacteria, max_pop)
        bacterial_pop = []
        for n in range(1, time_step + 1):
            bacterial_pop.append(patient.update())
            pops[i].append(len(bacterial_pop))

    x_vals = []
    for i in range(time_step):
        x_vals.append(i)
        average_pop.append(calc_pop_avg(pops, i))
...
class TreatedPatient(Patient):
    ...
    def update(self):
        ...
        #     try:
        #         child_bacteria.append(bacteria.reproduce(pop_density))
        #     except NoChildException:
        #         continue
...
def simulation_with_antibiotic(num_bacteria,
    ...
    for i in range(num_trials):
        ...
        bacteria_list = [bacteria] * num_bacteria
        ...
        for t in range(numStep):
            if t <= 150:
                populations[i][t] = patient.update()
...
# at a time
total_pop, resistant_pop = simulation_with_antibiotic(num_bacteria=100, max_pop=1000,
                                                      birth_prob=0.3,
                                                      death_prob=0.2,
                                                      resistant=False,
                                                      mut_prob=0.8,
